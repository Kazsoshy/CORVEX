...
function DashboardPage({ navigate }) {
  ...
  function GrowthBadge({ rate, label }) {
    if (rate === null || rate === undefined) return <span className="muted" style={{ fontSize: '0.8rem' }}>No prior data</span>;
    const up = rate >= 0;
    return (
      <span style={{
  ...
  // ── Derived values ────────────────────────────────────────────────────────
  const topBranch = data.branchSummary.length
    ? [...data.branchSummary].sort((a, b) => b.totalCollections - a.totalCollections)[0]
    : null;
  const bottomBranch = data.branchSummary.length
    ? [...data.branchSummary].sort((a, b) => a.totalCollections - b.totalCollections)[0]
    : null;
  ...
  return (
    <div className="page">
      ...
      */}
      <StatsGrid stats={[
        { label: 'Total Collections (This Month)',  value: formatCurrency(data.totalCollections) },
        { label: 'Total Sales (This Month)',         value: formatCurrency(data.totalSales) },
        { label: 'Total Outstanding Balance',        value: formatCurrency(data.totalOutstandingBalance) },
        { label: 'Overdue Accounts',                 value: String(data.overdueCount) },
        { label: 'Route Compliance',                 value: `${data.routeCompliance}%` },
        { label: 'Stock Alerts',                     value: String(data.stockAlerts) },
      ]} />
      ...
      <section className="panel content-panel">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, padding: '8px 0' }}>
          ...
          <div style={{ padding: 16, background: '#f8fafc', borderRadius: 10, border: '1px solid #e2e8f0' }}>
            ...
            <div style={{ fontSize: '1.6rem', fontWeight: 800, margin: '6px 0 4px', color: '#0f172a' }}>
              {data.salesGrowthRate !== null
                ? `${data.salesGrowthRate >= 0 ? '+' : ''}${data.salesGrowthRate}%`
                : '—'}
            </div>
          ...
          <div style={{ padding: 16, background: '#f8fafc', borderRadius: 10, border: '1px solid #e2e8f0' }}>
            ...
            <div style={{ fontSize: '1.6rem', fontWeight: 800, margin: '6px 0 4px', color: '#0f172a' }}>
              {data.collectionGrowthRate !== null
                ? `${data.collectionGrowthRate >= 0 ? '+' : ''}${data.collectionGrowthRate}%`
                : '—'}
            </div>
      ...
      {data.branchSummary.length > 1 && (
        <ChartCard
          title="Branch-Level Descriptive Comparison"
          subtitle="Collections, Sales, Outstanding Balance — Source: collection_payment, sales_invoices, customer_activity grouped by branch"
        >
          ...
          <table className="data-table">
            ...
            <tbody>
              {data.branchSummary.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', color: '#64748b' }}>No branch data available.</td></tr>
      ...
      {data.branchSummary.length > 1 && topBranch && bottomBranch && (
        <div className="grid two-up">
          <div className="analytics-card" style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 12, padding: 20 }}>
            <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#059669', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              Top Performer (by Collections)
            </span>
            <strong style={{ display: 'block', fontSize: '1.1rem', marginTop: 6 }}>{topBranch.branchName}</strong>
          ...
          <div className="analytics-card" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, padding: 20 }}>
            <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#dc2626', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              Lowest Collections
            </span>
            <strong style={{ display: 'block', fontSize: '1.1rem', marginTop: 6 }}>{bottomBranch.branchName}</strong>
            <span className="muted" style={{ fontSize: '0.85rem' }}>
      ...
      <section className="panel content-panel">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, padding: '8px 0' }}>
          {[
            { label: 'Active Collectors',    value: data.activeCollectors,       source: 'users, role=collector, status=Active' },
            { label: 'Active Sales Agents',  value: data.activeSalesAgents,      source: 'users, role=sales_staff, status=Active' },
            { label: 'Route Compliance',     value: `${data.routeCompliance}%`,  source: 'field_visits: Completed/Total × 100' },
            { label: 'Sales Visit Compl.',   value: `${data.salesVisitCompletion}%`, source: 'field_visits: type=Sales Completed/Total × 100' },
            { label: 'Total Customers',      value: data.totalCustomers,         source: 'customers table COUNT(*)' },
            { label: 'Overdue Accounts',     value: data.overdueCount,           source: 'customer_activity: outstanding_balance > 0' },
          ].map((m) => (
            <div key={m.label} style={{ padding: 14, background: '#f8fafc', borderRadius: 10, border: '1px solid #e2e8f0' }}>
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block' }}>{m.label}</span>
              <strong style={{ fontSize: '1.5rem', display: 'block', margin: '4px 0' }}>{m.value}</strong>
              <span className="muted" style={{ fontSize: '0.7rem' }}>{m.source}</span>
            </div>
          ))}
        </div>
      </section>
...
function PerformanceHistoryPanel() {
  ...
  if (loading) return (
    <section className="panel content-panel">
      ...
      <p className="muted" style={{ padding: '16px 0' }}>Loading performance history...</p>
    </section>
  );
  ...
  perfData.byBranch.forEach((branch) => {
    branch.data.forEach((pt) => {
      if (!periodMap[pt.period]) periodMap[pt.period] = { period: pt.period };
  ...
  perfData.byBranch.forEach((branch, i) => {
    lineKeys.push({ key: branch.branchName.replace(' Branch','') + ' Sales',   color: PALETTE[i*2   % PALETTE.length] });
    lineKeys.push({ key: branch.branchName.replace(' Branch','') + ' Colltns', color: PALETTE[(i*2+1)% PALETTE.length] });
  });
      ...
      {activeTab === 'overview' && (
        <>
          ...
          {branchBarData.length > 0 && (
            <ChartCard title="Branch Staff & Customer Overview" subtitle="Source: branches, users, customers, branch_inventory tables">
              <ResponsiveContainer width="100%" height={220}>
          ...
          <section className="panel content-panel">
            ...
            <div className="table-shell">
              <table className="data-table">
                ...
                <tbody>
                  {branches.length === 0 ? (
                    <tr><td colSpan={8} style={{ textAlign: 'center', color: '#64748b' }}>No branches found.</td></tr>
                  ) : branches.map((b) => (
                    <tr key={b.id}>
                      <td>
                        <strong>{b.branch_name}</strong>
                        <br /><span className="muted" style={{ fontSize: '0.75rem' }}>{b.address}</span>
                      </td>
                      <td>{b.region || '—'}</td>
                      <td>{b.phone || '—'}</td>
                      <td>{b.email || '—'}</td>
...
function buildAnalyticsParams(filters) {
  const params = { preset: filters.preset, compare_to_previous: 'true' };
...
function AnalyticsFilterBar({ filters, setFilters, branchOptions = [] }) {
...
function AnalyticalInsightsSection({ data, filters, branchOptions }) {
  const insights = useMemo(() => {
    const { summary: s, trends, lowStockItems, pendingVisits, branchSummary } = data || {};
    const safeLowStockItems = lowStockItems || [];
    const safePendingVisits = pendingVisits || [];
    const safeTrends = trends || { collections: [], sales: [] };
    ...
    const collectionPhrase =
      collectionGrowth === null || collectionGrowth === undefined
        ? 'is being recorded for the first time in this comparison window'
        : Math.abs(collectionGrowth) < SIG_PCT
          ? `held roughly steady at ${formatCurrency(s?.totalCollections || 0)}`
          : collectionGrowth > 0
            ? `rose to ${formatCurrency(s?.totalCollections || 0)}, up ${collectionGrowth.toFixed(1)}% from the previous period`
            : `fell to ${formatCurrency(s?.totalCollections || 0)}, down ${Math.abs(collectionGrowth).toFixed(1)}% from the previous period`;

    const salesPhrase =
      salesGrowth === null || salesGrowth === undefined
        ? 'is being recorded for the first time in this comparison window'
        : Math.abs(salesGrowth) < SIG_PCT
          ? `held roughly steady at ${formatCurrency(s?.totalSales || 0)}`
          : salesGrowth > 0
            ? `rose to ${formatCurrency(s?.totalSales || 0)}, up ${salesGrowth.toFixed(1)}% from the previous period`
            : `fell to ${formatCurrency(s?.totalSales || 0)}, down ${Math.abs(salesGrowth).toFixed(1)}% from the previous period`;

    const compliancePhrase =
      routeCompliance !== undefined && routeCompliance !== null
        ? `route compliance is at ${routeCompliance.toFixed(1)}%`
        : 'route compliance data is unavailable';

    const inventoryPhrase =
      inventoryHealth !== undefined && inventoryHealth !== null
        ? `inventory health stands at ${inventoryHealth.toFixed(1)}%`
        : 'inventory health data is unavailable';

    const summary =
      `${scopeLabel} recorded ${formatCurrency(s?.totalCollections || 0)} in collections and ${formatCurrency(s?.totalSales || 0)} in sales over the selected period. Collections ${collectionPhrase}; sales ${salesPhrase}. ${compliancePhrase}, and ${inventoryPhrase}. ` +
      `The overall performance score is ${performanceScore?.toFixed(0) || 'N/A'}/100, reflecting ${performanceScore >= 80 ? 'strong' : performanceScore >= 60 ? 'moderate' : 'challenging'} operational health across the organization.`;
    ...
    if (safeBranchOptions.length > 1) {
      ...
      const lowPerformer = safeBranchOptions
        .filter(b => (b.totalCollections || 0) < avgCollections * 0.7)
        .sort((a, b) => (a.totalCollections || 0) - (b.totalCollections || 0))[0];
      if (lowPerformer) {
        findings.push({
    ...
    if (attention.length === 0) {
      ...
      decisionText =
        `The most significant signal in this view is "${top.title.toLowerCase()}." ${top.text.split('.')[0]}. ` +
        `This may warrant attention to ${Array.from(decisionAreas).slice(0, 3).join(', ').toLowerCase()}, ` +
        `though the data should be reviewed alongside operational context before acting, since the analysis reflects correlation in the current period rather than a confirmed root cause.`;
    }
  ...
  return (
    <section className="panel content-panel" style={{ marginTop: 24, borderTop: '2px solid #e2e8f0', paddingTop: 20 }}>
      <div className="panel-section-header" style={{ marginBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <NavIcon name="brain" style={{ width: 20, height: 20, color: '#8b5cf6' }} />
          <div>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '1rem', fontWeight: 600, margin: '0 0 4px' }}>
              Operational Analysis & Insights
            </h3>
            <p className="muted" style={{ margin: 0, fontSize: '0.8rem', lineHeight: 1.5, maxWidth: 650 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12, marginBottom: 18 }}>
        ...
        <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 4, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <NavIcon name="search" style={{ width: 16, height: 16, color: '#2563eb' }} />
            <h4 style={{ fontSize: '0.8rem', fontWeight: 600, margin: 0, color: '#2563eb' }}>Key Findings</h4>
          </div>
          {insights.findings.length > 0 ? (
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 11 }}>
              {insights.findings.map((f, i) => (
                <li key={i} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                  <span style={{ width: 5, height: 5, borderRadius: '50%', marginTop: 7, flexShrink: 0, background: '#64748b' }} />
                  <span style={{ fontSize: '0.78rem', lineHeight: 1.55, color: '#64748b' }}>
                    <strong style={{ color: '#334155' }}>{f.label}:</strong> {f.text}
                  </span>
                </li>
              ))}
            </ul>
        ...
        {/* Attention Required */}
        <div style={{ background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 4, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <NavIcon name="alert-triangle" style={{ width: 16, height: 16, color: '#d97706' }} />
            <h4 style={{ fontSize: '0.8rem', fontWeight: 600, margin: 0, color: '#d97706' }}>Attention Required</h4>
          </div>
          {insights.attention.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {insights.attention.map((a, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 0', borderTop: i === 0 ? 'none' : '1px solid #fcd34d' }}>
                    <NavIcon name={IconName} style={{ width: 15, height: 15, color, flexShrink: 0, marginTop: 1 }} />
                    <div>
                      <p style={{ fontSize: '0.78rem', fontWeight: 600, margin: '0 0 3px', color }}>{a.title}</p>
                      <p style={{ fontSize: '0.78rem', color: '#78350f', lineHeight: 1.55, margin: 0 }}>{a.text}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {/* Positive Performance */}
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 4, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <NavIcon name="sparkles" style={{ width: 16, height: 16, color: '#059669' }} />
            <h4 style={{ fontSize: '0.8rem', fontWeight: 600, margin: 0, color: '#059669' }}>Positive Performance</h4>
          </div>
          {insights.positive.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {insights.positive.map((p, i) => (
                <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 0', borderTop: i === 0 ? 'none' : '1px solid #86efac' }}>
                  <NavIcon name="check-circle" style={{ width: 15, height: 15, color: '#059669', flexShrink: 0, marginTop: 1 }} />
                  <div>
                    <p style={{ fontSize: '0.78rem', fontWeight: 600, margin: '0 0 3px', color: '#059669' }}>{p.title}</p>
                    <p style={{ fontSize: '0.78rem', color: '#166534', lineHeight: 1.55, margin: 0 }}>{p.text}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 }}>
          <NavIcon name="lightbulb" style={{ width: 16, height: 16, color: '#0284c7' }} />
          <h4 style={{ fontSize: '0.85rem', fontWeight: 600, margin: 0, color: '#0284c7' }}>Decision Support</h4>
        </div>
        <p style={{ fontSize: '0.8rem', lineHeight: 1.65, color: '#0c4a6e', margin: '0 0 12px' }}>
          {insights.decisionText}
        </p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
          {insights.decisionAreas.map((tag, i) => (
            <span key={i} style={{
              fontSize: '0.7rem',
              padding: '5px 10px',
              borderRadius: 12,
...
function LiveOperationsDashboardPage({ navigate }) {
  const currentUser = getCurrentUser();
  ...
  const { data, loading, error } = useOperatingManagerAnalytics(filters);
  ...
  return (
    <div className="page">
      ...
      <section className="panel content-panel" style={{ marginBottom: 16 }}>
        <div className="panel-section-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <NavIcon name="layout-dashboard" style={{ width: 20, height: 20, color: '#2563eb' }} />
            <div>
        ...
        <div className="kpi-grid">
          {[
            { key: 'collections', label: 'Total Collections', value: formatCurrency(data.summary.totalCollections), delta: data.summary.collectionGrowthRate, deltaType: 'pct', good: 'up', icon: 'dollar-sign' },
            { key: 'sales', label: 'Total Sales', value: formatCurrency(data.summary.totalSales), delta: data.summary.salesGrowthRate, deltaType: 'pct', good: 'up', icon: 'shopping-cart' },
            { key: 'outstanding', label: 'Outstanding Balance', value: formatCurrency(data.summary.totalOutstandingBalance), delta: null, deltaType: 'pct', good: null, icon: 'credit-card' },
            { key: 'overdue', label: 'Overdue Accounts', value: String(data.summary.overdueCount), delta: null, deltaType: 'pct', good: 'down', icon: 'clock' },
            { key: 'compliance', label: 'Route Compliance', value: `${data.summary.routeCompliance}%`, delta: null, deltaType: 'pts', good: 'up', icon: 'route' },
            { key: 'inventory', label: 'Inventory Health', value: `${data.summary.inventoryHealth}%`, delta: null, deltaType: 'pts', good: 'up', icon: 'package' },
          ].map((k) => {
            const delta = k.delta;
              ...
              <div key={k.key} className="kpi-card">
                <div className="kpi-card-header">
                  <div className="kpi-card-label">
                    <NavIcon name={k.icon} style={{ width: 16, height: 16, color: '#64748b' }} />
                    {k.label}
                  </div>
                </div>
                ...
                <div className={`kpi-card-delta ${deltaClass}`}>
                  {delta !== null && Math.abs(delta) > 0.05 && (
                    <>
                      <NavIcon name={delta > 0 ? 'trending-up' : 'trending-down'} style={{ width: 14, height: 14 }} />
                      {deltaLabel}
                    </>
                  )}
                  <span style={{ color: '#94a3b8', marginLeft: 4 }}>vs prior period</span>
                </div>
              </div>
            );
          })}
      ...
      <section className="panel content-panel">
        <div className="panel-section-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <NavIcon name="bar-chart-2" style={{ width: 20, height: 20, color: '#2563eb' }} />
            <div>
              <h3>Period-over-Period Analysis</h3>
              <p className="muted" style={{ margin: '2px 0 0', fontSize: '0.82rem' }}>
        ...
        <div className="pop-grid">
          {(() => {
            ...
            return [
              ...
              <div key={card.key} className={`pop-card ${card.cardClass}`}>
                <div className="pop-card-header">
                  <div className="pop-card-label">
                    <NavIcon name={card.icon} style={{ width: 16, height: 16, color: '#64748b' }} />
                    {card.label}
                  </div>
                  <NavIcon name={card.trendIcon} style={{ width: 16, height: 16, color: card.cardClass === 'good' ? '#10b981' : card.cardClass === 'bad' ? '#ef4444' : '#94a3b8' }} />
                </div>
                <div className="pop-card-value">{card.value}</div>
                <div className={`pop-card-delta ${card.cardClass}`}>
                  {card.deltaType === '%' ? (
                    card.delta !== null ? `${card.delta >= 0 ? '+' : ''}${card.delta}%` : 'No prior period'
                  ) : (
                    card.delta
                  )}
                </div>
      ...
      <div className="grid two-up">
        <section className="panel content-panel">
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="line-chart" style={{ width: 18, height: 18, color: '#2563eb' }} />
              <div>
                <h3>Collections Trend</h3>
                <p className="muted" style={{ margin: '2px 0 0', fontSize: '0.82rem' }}>{data.filters.label} vs prior period</p>
              </div>
            </div>
          </div>
          <p className="muted" style={{ fontSize: '0.82rem', marginTop: 0, marginBottom: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={230}>
            <LineChart data={trendCollections}>
              ...
              <XAxis dataKey="label" tick={{ fill: '#64748b', fontSize: 11 }} axisLine={{ stroke: '#e2e8f0' }} tickLine={false} minTickGap={28} />
              <YAxis tick={{ fill: '#64748b', fontSize: 11 }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} width={30} />
              <Tooltip contentStyle={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 4, fontSize: 12 }} labelStyle={{ color: '#1e293b' }} formatter={(v) => formatCurrency(v)} />
        ...
        <section className="panel content-panel">
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="line-chart" style={{ width: 18, height: 18, color: '#10b981' }} />
              <div>
                <h3>Sales Trend</h3>
                <p className="muted" style={{ margin: '2px 0 0', fontSize: '0.82rem' }}>{data.filters.label} vs prior period</p>
              </div>
          ...
          <div className="analysis-box analysis-box-green">
            <p style={{ margin: 0 }}>
              {(() => {
                ...
                const growth = ((currentTotal - previousTotal) / previousTotal * 100).toFixed(1);
                const isGrowth = Number(growth) >= 0;
                const trend = trendSales.length > 2 ?
                  (trendSales[trendSales.length - 1].amount > trendSales[0].amount ? 'upward' :
                   trendSales[trendSales.length - 1].amount < trendSales[0].amount ? 'downward' : 'stable') : 'stable';

                return isGrowth
                  ? `Sales are ${growth}% higher (Current: ${formatCurrency(currentTotal)} vs Previous: ${formatCurrency(previousTotal)}; Growth = (${formatCurrency(currentTotal)} - ${formatCurrency(previousTotal)}) / ${formatCurrency(previousTotal)} × 100) with a ${trend} trend, indicating strong market demand and effective product offerings. This suggests customer confidence and healthy business growth.`
      ...
      <section className="panel content-panel">
        <div className="panel-section-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <NavIcon name="bar-chart-3" style={{ width: 18, height: 18, color: '#8b5cf6' }} />
            <div>
              <h3>Branch Comparison</h3>
              <p className="muted" style={{ margin: '2px 0 0', fontSize: '0.82rem' }}>Collections, sales, balance, compliance, and inventory health</p>
            </div>
          </div>
        </div>
        <p className="muted" style={{ fontSize: '0.82rem', marginTop: 0, marginBottom: 16 }}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={branchOptions.map((branch) => ({
            ...
            <YAxis tick={{ fill: '#64748b', fontSize: 11 }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} width={28} />
            <Tooltip contentStyle={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 4, fontSize: 12 }} labelStyle={{ color: '#1e293b' }} formatter={(v) => formatCurrency(v)} />
            <Legend wrapperStyle={{ fontSize: 12, color: '#64748b' }} />
      ...
      <div className="grid two-up">
        <section className="panel content-panel">
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="building-2" style={{ width: 18, height: 18, color: '#2563eb' }} />
              <div>
        ...
        <section className="panel content-panel">
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="alert-triangle" style={{ width: 18, height: 18, color: '#f59e0b' }} />
              <div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {data.lowStockItems.slice(0, 5).map((item, index) => (
                  <tr key={`${item.branchName}-${item.productName}-${index}`}>
                    <td>Low stock</td>
                    <td>{item.branchName}</td>
                    <td>{item.productName} · {item.availableStock} remaining</td>
                  </tr>
                ))}
                {data.pendingVisits.slice(0, 5).map((visit) => (
      ...
      <div className="grid two-up">
        <section className="panel content-panel">
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="users" style={{ width: 18, height: 18, color: '#2563eb' }} />
              <div>
                <h3>Top Customers</h3>
                <p className="muted" style={{ margin: '2px 0 0', fontSize: '0.82rem' }}>
        ...
        <section className="panel content-panel">
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="package" style={{ width: 18, height: 18, color: '#10b981' }} />
              <div>
                <h3>Top Products</h3>
                <p className="muted" style={{ margin: '2px 0 0', fontSize: '0.82rem' }}>
      ...
      {topBranch && bottomBranch ? (
        <section className="panel content-panel" style={{ marginTop: 16 }}>
          <div className="panel-section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <NavIcon name="trophy" style={{ width: 18, height: 18, color: '#f59e0b' }} />
              <div>
            ...
            <div className="performance-card good">
              <div className="performance-card-header">
                <div className="performance-card-badge good">
                  <NavIcon name="award" style={{ width: 14, height: 14 }} />
                  Top Branch
                </div>
              </div>
              ...
              <p className="muted" style={{ fontSize: '0.75rem', marginTop: 8, marginBottom: 0, color: '#64748b' }}>
            ...
            <div className="performance-card bad">
              <div className="performance-card-header">
                <div className="performance-card-badge bad">
                  <NavIcon name="alert-circle" style={{ width: 14, height: 14 }} />
                  Needs Attention
                </div>
              </div>
...
function LiveBranchComparisonPage({ navigate }) {
  const [filters, setFilters] = useState({ preset: 'this_month', startDate: '', endDate: '', branchId: 'all' });
  const { data, loading, error } = useOperatingManagerAnalytics(filters);
    ...
    <div className="page">
      ...
      <div className="grid two-up">
        <ChartCard title="Collections by Branch" subtitle="Current filtered period">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={(data.branchSummary || []).map((branch) => ({ name: branch.branchName, value: branch.totalCollections }))}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="name" tick={{ fontSize: 12 }} />
              <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
              <Tooltip formatter={(v) => formatCurrency(v)} />
        ...
        <ChartCard title="Sales by Branch" subtitle="Current filtered period">
          <ResponsiveContainer width="100%" height="240">
            <BarChart data={(data.branchSummary || []).map((branch) => ({ name: branch.branchName, value: branch.totalSales }))}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="name" tick={{ fontSize: 12 }} />
              <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
              <Tooltip formatter={(v) => formatCurrency(v)} />
      ...
      <PageToolbar actions={[{ label: 'Back to Dashboard', to: '/operating-manager/dashboard', variant: 'ghost' }]} onAction={(action) => navigate(action.to)} />
    </div>
  );
}

function LiveReportsPage({ navigate, showToast }) {
  const [filters, setFilters] = useState({ preset: 'this_month', startDate: '', endDate: '', branchId: 'all' });
  const [activeTab, setActiveTab] = useState('collections');
  const { data, loading, error } = useOperatingManagerAnalytics(filters);
    ...
    <div className="page">
      ...
      <div className="segmented-control" style={{ marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          ...
          <button key={tab.key} className={activeTab === tab.key ? 'segment active' : 'segment'} type="button" onClick={() => setActiveTab(tab.key)}>
            {tab.label}
          </button>
        ))}
      </div>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {data.lowStockItems.map((item) => (
                    <tr key={`${item.branchName}-${item.productName}`}>
                      <td>{item.productName}</td><td>{item.branchName}</td><td>{item.availableStock}</td><td><SeverityBadge severity={item.status === 'Out of Stock' ? 'Critical' : 'Warning'} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'executive' && (
        <div className="grid two-up">
          <ChartCard title="Executive Snapshot" subtitle="Key operational indicators">
            <StatsGrid stats={[
              { label: 'Collections', value: formatCurrency(data.summary.totalCollections) },
              { label: 'Sales', value: formatCurrency(data.summary.totalSales) },
              { label: 'Outstanding', value: formatCurrency(data.summary.totalOutstandingBalance) },
              { label: 'Inventory Health', value: `${data.summary.inventoryHealth}%` },
            ]} />
          </ChartCard>
      ...
      <PageToolbar actions={[{ label: 'Export PDF', variant: 'secondary' }, { label: 'Export Excel', variant: 'secondary' }]} onAction={(action) => showToast(`${action.label} started.`, 'success')} />
      <PageToolbar actions={[{ label: 'Back to Dashboard', to: '/operating-manager/dashboard', variant: 'ghost' }]} onAction={(action) => navigate(action.to)} />
    </div>
  );
}
...
function LiveHistoricalTrendsPage() {
  const [filters, setFilters] = useState({ preset: 'this_year', startDate: '', endDate: '', branchId: 'all' });
  const { data, loading, error } = useOperatingManagerAnalytics(filters);
...
function LiveLeafletPage({ navigate, subPage }) {
  const [branches, setBranches] = useState([]);
  const [loading, setLoading] = useState(true);
  const [filters, setFilters] = useState({ branch: 'All Branches', dateRange: 'This Month', staffType: 'All Staff' });
...
export function OperatingManagerPageBody({ page, navigate, showToast }) {
  ...
  switch (page.pageType) {
    case 'dashboard': return <LiveOperationsDashboardPage {...props} />;
    case 'branchPerformance': return <LiveBranchComparisonPage {...props} />;
    case 'branchComparison': return <LiveBranchComparisonPage {...props} />;
    case 'branchDetail': return <BranchDetailPage {...props} />;
    case 'historicalTrends': return <LiveHistoricalTrendsPage {...props} />;
    case 'leafletMap': return <LiveLeafletPage {...props} />;
    case 'leafletDelinquency': return <LiveLeafletPage {...props} subPage="delinquency" />;
    case 'leafletProfitability': return <LiveLeafletPage {...props} subPage="profitability" />;
    case 'leafletTerritory': return <LiveLeafletPage {...props} subPage="territory" />;
    case 'reports':
    case 'reportCollections':
    case 'reportSales':
    case 'reportInventory':
    case 'reportDelinquency':
    case 'reportExecutive': return <LiveReportsPage {...props} />;
    ...
    case 'territories': return <TerritoriesPage {...props} />;

...
import { CUSTOMER_ACCOUNT, NOTIFICATIONS, PAYMENTS, RECEIPTS, STATEMENTS, UPCOMING_DUE_DATES, formatCurrency, getReceiptById, getStatementById } from '../../data/customerMockData';
...
function LoginPage({
  ...
  return <div className="page customer-login-page">
      <section className="panel form-panel narrow customer-login-panel">
        ...
        <label>
          Account Number
          <input value={form.accountNumber} onChange={e => setForm(f => ({
          ...f,
        ...
        <label>
          ...
          <input type={form.useOtp ? 'text' : 'password'} value={form.password} onChange={e => setForm(f => ({
          ...f,
          password: e.target.value
        }))} placeholder={form.useOtp ? 'Enter OTP code' : 'Enter password'} autoComplete={form.useOtp ? 'one-time-code' : 'current-password'} />
          {errors.password ? <span className="form-error">{errors.password}</span> : null}
        </label>

        <label className="toggle-label">
          <input type="checkbox" checked={form.useOtp} onChange={e => setForm(f => ({
          ...f,
          useOtp: e.target.checked,
          password: ''
        }))} />
        ...
        <label className="toggle-label">
          <input type="checkbox" checked={form.rememberDevice} onChange={e => setForm(f => ({
          ...f,
        ...
        <div className="form-actions">
          ...
          {form.useOtp ? <button className="inline-flex justify-center items-center gap-2 px-5 py-2.5 rounded-md bg-transparent text-blue border-[1.5px] border-blue-30 shadow-none hover:bg-blue-08 transition-all duration-160 cursor-pointer" type="button" onClick={() => showToast('OTP resent to registered contact.', 'success')}>Resend OTP</button> : null}
        </div>
      </section>
    </div>;
}
function HomePage({
  ...
  return <div className="page customer-page">
      ...
      value: formatCurrency(CUSTOMER_ACCOUNT.outstandingBalance)
    }, {
      label: 'Next Due Date',
      value: 'July 5, 2026'
    }, {
      label: 'Account Status',
      value: CUSTOMER_ACCOUNT.accountStatus
    }]} />
      ...
      <div className="dashboard-widgets grid two-up">
        <section className="panel content-panel relative overflow-hidden">
          ...
          {recentPayments.length ? <ul className="list-none p-0 m-0 flex flex-col gap-3">
              {recentPayments.map(p => <li key={p.id}><div><strong>{formatCurrency(p.amount)}</strong><span className="text-ink/70">{p.date}</span></div><span>{p.receiptNumber}</span></li>)}
            </ul> : <EmptyState title="No payments yet" description="Your payment history will appear here." />}
        </section>
        <section className="panel content-panel relative overflow-hidden">
          ...
          <ul className="list-none p-0 m-0 flex flex-col gap-3">
            {UPCOMING_DUE_DATES.map(d => <li key={d.date}><div><strong>{formatCurrency(d.amount)}</strong><span className="text-ink/70">{d.label}</span></div><span>{d.date}</span></li>)}
          </ul>
        </section>
      </div>
      ...
      <section className="panel content-panel relative overflow-hidden">
        ...
        {filtered.length === 0 ? <EmptyState title="No payments found" description="Try adjusting your date range or payment type filter." /> : <><div className="corvex-table-wrapper">
            <table className="corvex-table">
              ...
              <tbody>
                {paginated_filtered.map(p => <tr key={p.id}>
                    <td>{p.date}</td>
...
function ReceiptsPage({
  ...
  return <div className="page customer-page">
      {RECEIPTS.length === 0 ? <EmptyState title="No receipts" description="Receipts will appear here after payments are confirmed." /> : <section className="panel content-panel relative overflow-hidden">
          <><div className="corvex-table-wrapper">
            <table className="corvex-table">
              <thead><tr><th>Receipt #</th><th>Date</th><th>Amount</th><th>Actions</th></tr></thead>
              <tbody>
                {paginated_RECEIPTS.map(r => <tr key={r.id}>
...
function StatementsPage({
  ...
  const pagination_STATEMENTS = usePagination(STATEMENTS);
  ...
  return <div className="page customer-page">
      ...
      {STATEMENTS.length === 0 ? <EmptyState title="No statements" description="Monthly statements will appear here." /> : <section className="panel content-panel relative overflow-hidden">
          ...
          <><div className="corvex-table-wrapper">
            <table className="corvex-table">
              ...
              <tbody>
                {paginated_STATEMENTS.map(s => <tr key={s.id}>
                    <td>{s.month}</td>
